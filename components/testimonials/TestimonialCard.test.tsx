...
describe('TestimonialCard', () => {
  it('renders testimonial with author name and content', () => {
    ...
    expect(screen.getByText(/Excellente expérience, je recommande vivement!/)).toBeInTheDocument();
  });
  ...
  it('displays location when provided', () => {
    render(<TestimonialCard authorName="S.L." content="Super!" rating={5} location="Evian" />);

    expect(screen.getByText('Evian')).toBeInTheDocument();
  });

  it('displays verified badge when verified is true', () => {
    render(<TestimonialCard authorName="A.B." content="Parfait" rating={5} verified />);

    expect(screen.getByText('Vérifié')).toBeInTheDocument();
  });

  it('does not display verified badge when verified is false', () => {
    render(<TestimonialCard authorName="C.D." content="Bien" rating={4} verified={false} />);
  ...
  it('applies custom className', () => {
    const { container } = render(
      <TestimonialCard authorName="E.F." content="Nice" rating={5} className="custom-class" />
    );
  ...
  it('renders with minimum required props', () => {
    render(<TestimonialCard authorName="G.H." content="Good service" rating={3} />);
